...
Inputs: None
Output: (int)number1, (int)number 2, (string)operator
...
def get_valid_expression_inputs():
    while True:
        ...
        if y not in ['+', '-', '*', '/']:
            ...
            continue
        break
    #order doesnt matter how they are listed
    return x, z, y

'''
Funtion to evaluate expression
Inputs: (int)x, (int)z, (string)y (y = the operator)
Output: (int)answer
'''
def evaluate_expressison(x, z, y):
    if y == '+':
        answer = x + z
    elif y == '-':
        answer = x - z
...
def main():
    while True:
        #call get_valid_expression_inputs function to get the x, z, and operator (y) values
        x, z, y = get_valid_expression_inputs()
        
        #call evaluate_expression to get the answer for the expression
        answer = evaluate_expressison
        #error here 
        print(f"{answer:.1f}")
...
#whenever you want to call a funtion, you must use () to call the fucntion
